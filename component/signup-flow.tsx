...
export default function SignupFlow({ onBackToLogin }: SignupFlowProps) {
  ...
  const handleContinue = async (newData: Partial<FormData>) => {
    setFormData({ ...formData, ...newData });
    setError(null);

    // If we're on step 3 (Verify), submit the form
    if (currentStep === 3) {
      await handleSignup({ ...formData, ...newData });
    } else if (currentStep < 4) {
      setCurrentStep(currentStep + 1);
    }
  };

  const handleSignup = async (finalData: FormData) => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      if (result.success) {
        ...
        // If user is automatically logged in, redirect to dashboard after a moment
        ...
        // Stay on verify step to show error
      }
    } catch (err: any) {
